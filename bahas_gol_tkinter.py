...
class Field:
    ...
    def helper(self,r,c):
        ...
        if self.a[r][c] == 2:
            if cnt_foxes > 1: # if foxes are at least 2 and there is one rabbit they can eat it and reproduce to its place
                return 1
    ...
    def step(self):
        ...
        for r in range(self.n):
            for c in range(self.m):
                tmp[r][c] = self.helper(r,c) # temporary array filled with next moves of animals
        self.a = tmp
...
try:
    fox = Fox()
    rabbit = Rabbit()
    grass = Grass()      # Exception handling to ensure that objects are properly initialized
except:
